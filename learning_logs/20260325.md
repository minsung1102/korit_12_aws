...
Nginx - Certbot까지 적용하시면 https로 적용이 가능
...
## RDS 인스턴스
AWS로부터 빌린 DB가 설치되어있는 컴퓨터 한 대를 RDS 인스턴스라고 합니다. EC2ㅘ 같습니다.
- 우리는 localhost로 실행할 때 컴퓨터 한 대에서 backend 서버와 DB를 동시에 돌렸는데, 그 말은 EC2 내부에 DB를 설치해서 사용하는 방법이 있지 않을까요? -> 가능.
- 이유는 이하에서 설명하겠습니다.
...
3. 스토리지 : RDS 인스턴스 컴퓨터이기 때문에 저장 공간이 존재합니다. EC2와 용어 동일.

## RDS를 사용하는 이유
1. EC2로 백엔드 배포를 하고, DB는 본인 컴퓨터에 설치해서 쓰는 방법도 가능.
2. EC2에 백엔드 및 DB를 배포하는 방법 
  - 반드시 RDS를 활용할 필요는 없습니다. 토이 프로젝트의 경우에는 가능할 것 같습니다.(대신 create-drop 해두면 더미 데이터를 미리 많이 CommandLineRunner를 통해 집어넣어둬야겠네요). 실무에서 권장하지 않습니다. 만약에 백엔드 서버에 장애 바생 시에 EC2인스턴스에 이상이 생길 경우 DB도 영향을 받을 수 있기 때문입니다.
...
- DB 파라미터 그룹을 변경한 후에는 RDS를 재부팅해야만 정상 작동이 됩니다(근데 우리 이거 생성하는데 시간 걸렸기 때문에 빠르게 나갔습니다).

## RDS로 접속하기
- 여러분 컴퓨터에 DBeaver 설치 여부 확인 -> 안되어있으면 설치
- SQL 학습할 때 있었으니까 SQL repository 내에 있겠네요

- instargram-db1 인스턴스로 들어가서 연결 부분을 엔드포인트로 지정했습니다.
- dbeaver 켠 다음에 데이터베이스 연결 -> MYSQL 선택 후에 server name에 aws의 엔드포인트를 붙여넣고, username / password를 인스턴스 설정 시 만들었던 master id 와 master password를 입력했습니다.
...
- EC2 인스턴스 생성했습니다. instargram-rds-server / key pair / security group 생성하고 연결했습니다.
...
- application.yml 확인 -> port 8080인지 확인
- 80이면 -> 보안그룹이 8080인지 확인
- 8080이면 -> 보안그룹이 80인지 확인

## 안전 삭제
- EC2 - RDS 간의 연결은 : application.yml을 통해서만 이루어져있고 인스턴스 추가 등 AWS에서 실행한 적이 없습니다.
